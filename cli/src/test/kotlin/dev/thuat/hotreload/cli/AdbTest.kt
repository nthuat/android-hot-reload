...
class AdbTest {
    ...
    fun `runAsCopy shape - mkdir then cp wrapped as one sh -c argv token`() {
        val fake = FakeRunner()
        Adb("adb", null, fake).runAsCopy("dev.thuat.hotreload.sample", "/data/local/tmp/hotreload/agent.so", "hotreload/agent.so")
        val argv = fake.calls.single()
        assertEquals(listOf("adb", "shell", "run-as", "dev.thuat.hotreload.sample", "sh", "-c"), argv.dropLast(1))
    ...
    fun `attachAgent uses am attach-agent`() {
        val fake = FakeRunner()
        Adb("adb", null, fake).attachAgent("dev.thuat.hotreload.sample", "/data/data/dev.thuat.hotreload.sample/code_cache/hotreload/agent.so")
        assertEquals(
            listOf("adb", "shell", "am", "attach-agent", "dev.thuat.hotreload.sample",
                "/data/data/dev.thuat.hotreload.sample/code_cache/hotreload/agent.so"),
            fake.calls.single(),
        )
    }
