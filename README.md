...
`./hotreload` is generated by `hotReloadInstallCli` with your project path and `applicationId`
baked in, and is rewritten on every run. Add `/hotreload` and `/.hotreload/` to `.gitignore`.
</details>
...
- `ANDROID_HOME` set, with `platform-tools` on it.
